    ...
    var Table = {
        ...
        api: {
            ...
            bindevent: function (table) {
                ...
                // 添加按钮事件1
                $(toolbar).on('click', Table.config.testbtn, function () {

                    var ids = Table.api.selectedids(table);
                    var url = options.extend.test_url;
                    if (url.indexOf("{ids}") !== -1) {
                        url = Table.api.replaceurl(url, {ids: ids.length > 0 ? ids.join(",") : 0}, table);
                    }
                    Fast.api.open(url, __('tesa'), $(this).data() || {});
                });
                // 添加按钮事件2
                $(toolbar).on('click', Table.config.testabtn, function () {

                    var ids = Table.api.selectedids(table);
                    var url = options.extend.testb_url;
                    if (url.indexOf("{ids}") !== -1) {
                        url = Table.api.replaceurl(url, {ids: ids.length > 0 ? ids.join(",") : 0}, table);
                    }
                    Fast.api.open(url, __('test'), $(this).data() || {});
                });
                // 添加按钮事件 testb
                $(toolbar).on('click', Table.config.testbtn, function () {

                    var ids = Table.api.selectedids(table);
                    var url = options.extend.testa_url;
                    if (url.indexOf("{ids}") !== -1) {
                        url = Table.api.replaceurl(url, {ids: ids.length > 0 ? ids.join(",") : 0}, table);
                    }
                    Fast.api.open(url, __('test'), $(this).data() || {});
                ...
                // 批量编辑修改状态的按钮事件
                $(toolbar).on('click', Table.config.changememobtn, function () {
                    var that = this;
                    //循环弹出多个编辑框
                    $.each(table.bootstrapTable('getSelections'), function (index, row) {
                        var url = options.extend.changememo_url;
                        row = $.extend({}, row ? row : {}, {ids: row[options.pk]});
                        var url = Table.api.replaceurl(url, row, table);
                        Fast.api.open(url, __('Changememo'), $(that).data() || {});
                    });
                });
                ...
                //添加状态编辑按钮
                $(table).on("click", "[data-id].btn-changememo", function (e) {
                    e.preventDefault();
                    var ids = $(this).data("id");
                    var row = Table.api.getrowbyid(table, ids);
                    row.ids = ids;
                    var url = Table.api.replaceurl(options.extend.edit_url, row, table);
                    Fast.api.open(url, __('Changememo'), $(this).data() || {});
                });
